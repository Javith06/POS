...
export default function CartSidebar({ width = 400 }: CartSidebarProps) {
  ...
  const tableData = useMemo(() => {
    ...
    if (orderContext.orderType === "TAKEAWAY") {
      return tables.find(t => t.section === "TAKEAWAY" && t.tableNo === orderContext.takeawayNo);
    }
  ...
  const handleCheckout = () => {
    ...
    if (orderContext.orderType === "DINE_IN") {
      updateTableStatus(orderContext.section!, orderContext.tableNo!, activeOrder?.orderId || "PAYMENT", 'BILL_REQUESTED', undefined, undefined, payableAmount);
  ...
  const handleSendOrder = () => {
    ...
    if (orderContext.orderType === "DINE_IN") {
      updateTableStatus(orderContext.section!, orderContext.tableNo!, targetOrderId, 'SENT', undefined, undefined, payableAmount);
      router.replace(`/(tabs)/category?section=${orderContext.section}`);
    } else {
      updateTableStatus("TAKEAWAY", orderContext.takeawayNo!, targetOrderId, 'SENT', undefined, undefined, payableAmount);
      router.replace(`/(tabs)/category?section=TAKEAWAY`);
    }
    clearCart();
    showToast({ type: "success", message: "Order Sent", subtitle: "Kitchen has been notified." });
  };
      ...
      <View style={[styles.itemContainer, isExpanded && styles.itemExpanded]}>
        ...
        >
          <View style={styles.itemIndexWrap}>
            <Ionicons name={isExpanded ? "chevron-down" : "chevron-forward"} size={12} color={Theme.textMuted} style={styles.chevron} />
            <Text style={styles.itemIndex}>{index + 1}</Text>
          </View>
          
          <View style={styles.itemInfo}>
            <View style={styles.itemMainRow}>
              <Text style={[styles.itemName, isSent && styles.textMuted]} numberOfLines={1}>{item.name}</Text>
              <View style={[styles.statusTag, { backgroundColor: isSent ? "#22C55E15" : "#3B82F615" }]}>
                 <Text style={[styles.statusTagText, { color: isSent ? "#15803D" : "#1D4ED8" }]}>
                   {isSent ? "✅ SENT" : "🔵 NEW"}
                 </Text>
              </View>
            </View>
          ...
          <View style={styles.discountRow}>
             <View style={styles.discountInputWrap}>
               ...
               <TextInput 
                 ...
                 onChangeText={(txt) => updateCartItemQty(item.lineItemId, item.qty, parseInt(txt) || 0)}
                 selectTextOnFocus
                 maxLength={3}
               />
             </View>
      ...
      <View style={styles.footer}>
        ...
        <View style={styles.actions}>
           {cart.length > 0 ? (
             ...
             <TouchableOpacity 
               style={[styles.proceedBtn, { flex: 1, backgroundColor: "#F59E0B" }]}
      ...
      <Modal transparent visible={showCancelModal} animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TextInput style={styles.modalInput} secureTextEntry autoFocus value={cancelPassword} onChangeText={setCancelPassword} placeholder="Admin Password" />
            <View style={styles.modalActions}>
              <TouchableOpacity style={styles.modalBtnCancel} onPress={() => setShowCancelModal(false)}><Text style={styles.modalBtnTextCancel}>Back</Text></TouchableOpacity>
              <TouchableOpacity style={styles.modalBtnConfirm} onPress={() => cancelPassword === "786" && clearCart()}><Text style={styles.modalBtnTextConfirm}>Confirm</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
...
const styles = StyleSheet.create({
  container: { height: "100%", backgroundColor: Theme.bgCard, borderLeftWidth: 1, borderLeftColor: Theme.border, padding: 16 },
  emptySurface: { flex: 1, justifyContent: "center", alignItems: "center", opacity: 0.5 },
  emptyText: { fontFamily: Fonts.bold, color: Theme.textMuted, marginTop: 16, fontSize: 18 },
  header: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 20 },
  tableIdentity: { flex: 1 },
  tableIdentityText: { fontSize: 16, fontFamily: Fonts.extraBold, color: Theme.textPrimary, textTransform: "uppercase" },
  headerIcons: { flexDirection: "row", gap: 8 },
  headerIconButton: { width: 36, height: 36, borderRadius: 10, backgroundColor: Theme.bgMuted, justifyContent: "center", alignItems: "center" },
  listContent: { paddingBottom: 20 },
  itemContainer: { marginBottom: 12, borderRadius: 12, backgroundColor: Theme.bgCard, overflow: "hidden", flexDirection: "row" },
  ...
  qtyBtnSmall: { width: 30, height: 30, justifyContent: "center", alignItems: "center", backgroundColor: "#fff", borderRadius: 6, ...Theme.shadowSm },
  qtyTextSmall: { paddingHorizontal: 12, fontSize: 13, fontFamily: Fonts.extraBold, color: Theme.textPrimary },
  priceContainer: { flexDirection: "row", alignItems: "center", gap: 8 },
  itemPrice: { fontSize: 14, fontFamily: Fonts.extraBold, color: Theme.primary, minWidth: 60, textAlign: "right" },
  deleteBtn: { padding: 4, marginLeft: 4 },
  textMuted: { color: Theme.textMuted },
  discountRow: { padding: 10, backgroundColor: Theme.bgMain + "30", borderTopWidth: 1, borderTopColor: Theme.border + "50" },
  discountInputWrap: { flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  discountLabel: { fontSize: 11, fontFamily: Fonts.bold, color: Theme.textMuted },
  discountInputSmall: { width: 60, height: 32, backgroundColor: "#fff", borderRadius: 6, borderWidth: 1, borderColor: Theme.border, textAlign: "center", fontSize: 12, fontFamily: Fonts.black, ...Platform.select({ web: { outlineStyle: "none" } as any }) },
  footer: { borderTopWidth: 1, borderTopColor: Theme.border, paddingTop: 16 },
  addBar: { flexDirection: "row", alignItems: "center", backgroundColor: Theme.primaryLight, padding: 10, borderRadius: 10, marginBottom: 16 },
  addLabel: { fontSize: 12, fontFamily: Fonts.black, color: Theme.primary, marginRight: 15 },
  ...
  summaryLabel: { fontSize: 13, fontFamily: Fonts.medium, color: Theme.textSecondary },
  summaryValue: { fontSize: 14, fontFamily: Fonts.black, color: Theme.textPrimary },
  payableLabel: { fontSize: 15, fontFamily: Fonts.black, color: Theme.textPrimary },
  payableValue: { fontSize: 20, fontFamily: Fonts.black, color: Theme.textPrimary },
  actions: { flexDirection: "row", gap: 10 },
  holdBtn: { flex: 1, height: 50, backgroundColor: "#F97316", borderRadius: 12, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, ...Theme.shadowMd },
  proceedBtn: { flex: 1.1, height: 50, backgroundColor: "#F59E0B", borderRadius: 12, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, ...Theme.shadowMd },
  btnText: { color: "#fff", fontFamily: Fonts.black, fontSize: 15 },
  modalOverlay: { flex: 1, backgroundColor: "rgba(0,0,0,0.5)", justifyContent: "center", alignItems: "center" },
  modalContent: { backgroundColor: Theme.bgCard, padding: 24, borderRadius: 20, width: 300 },
  modalTitle: { fontSize: 18, fontFamily: Fonts.black, marginBottom: 15 },
  modalInput: { borderWidth: 1, borderColor: Theme.border, borderRadius: 10, padding: 12, marginBottom: 20 },
  modalActions: { flexDirection: "row", justifyContent: "flex-end", gap: 10 },
  modalBtnCancel: { padding: 10 },
  modalBtnTextCancel: { color: Theme.textSecondary, fontFamily: Fonts.bold },
  modalBtnConfirm: { backgroundColor: Theme.danger, paddingHorizontal: 20, paddingVertical: 10, borderRadius: 10 },
